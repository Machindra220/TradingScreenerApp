...
UPLOAD_FOLDER = os.path.abspath(os.path.join(os.getcwd(), 'uploads', 'ibd_us'))
...
def calculate_ad_rating(df, lookback=20):
    """Calculates an IBD-style Accumulation/Distribution Score from A to E for US markets."""
    if len(df) < lookback:
        return "C"
...
def compute_weighted_composite_score(rs_pct, eps_score, ad_grade):
    """Combines technological percentile ranks and fundamental EPS into a 1-99 scale."""
...
def run_ibd_us_screening_pipeline(symbols):
    try:
        # Fetch S&P 500 benchmark baseline index and immediately clear timezone offsets
        index_data = yf.download("^GSPC", period="1y", interval="1d", auto_adjust=True)
        if index_data.empty:
            print("❌ Failed to fetch broad-market benchmark index data (^GSPC).")
            return []
    ...
    fundamentals = {}
    mock_fundamentals_path = os.path.join(UPLOAD_FOLDER, 'mock_fundamentals_us.json')
    ...
    raw_candidates = []
    print(f"📥 Fetching US market data matrices for {len(symbols)} tickers...")
    ...
    for sym in symbols:
        try:
            ...
            # Calculate Technical RS Line Slope Intensity vs S&P 500 Index Base
            ...
            eps_score = fundamentals.get(sym, {}).get("eps_rating", 75)
...
def ibd_us_scan_process():
    ...
    return render_template("ibd_smartselect_us.html", stocks=stocks, last_time=last_time)

@ibd_engine_us_bp.route("/export-ibd-us")
def export_ibd_us():
    """✅ EXPORT CHANNEL FEATURE: Packages active data layers into timestamped standalone CSVs"""
    if os.path.exists(RESULTS_JSON):
        with open(RESULTS_JSON, 'r') as f:
        ...
        if stocks:
            ...
            temp_path = os.path.join(UPLOAD_FOLDER, 'temp_us_ibd_export.csv')
            df.to_csv(temp_path, index=False)
            return send_file(temp_path, as_attachment=True, download_name=f"US_IBD_SmartSelect_{timestamp}.csv")
