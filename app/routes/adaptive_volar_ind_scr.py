...
# Index: ^NSEI (Nifty 50) is used because it's the most reliable yfinance ticker for India.
# Nifty 500 (^CRSLDX) would be more representative for broad-market RS but has
# inconsistent history on yfinance — switch to ^CRSLDX if your yfinance version supports it.
INDEX_TICKER = "^NSEI"
...
def is_volar_adaptive_ind(symbol):
    """
    ...
    try:
        ...
        print(f"Processing: {symbol} → {ticker_symbol}")
        ...
        if len(df) < LB_6M or len(idx_df) < LB_6M:
            print(f"  Skipping {ticker_symbol}: insufficient data (need {LB_6M}, got {len(df)})")
            return None
        ...
        if curr <= ema200:
            print(f"  Skipping {ticker_symbol}: below EMA200 ({curr:.2f} <= {ema200:.2f})")
            return None
...
def volar_ind_process():
    ...
    if request.method == "POST":
        ...
        if saved_path and os.path.exists(saved_path):
            ...
            raw_results = [r for r in (is_volar_adaptive_ind(s) for s in symbols) if r]
