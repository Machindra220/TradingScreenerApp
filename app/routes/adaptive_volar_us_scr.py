...
def is_volar_adaptive(symbol, lookback):
    try:
        ...
        print(f"Data length for {symbol}: {len(df)}")
        print(f"Index data length: {len(idx_df)}")

        if len(df) < lookback or len(idx_df) < lookback:
            print(f"Not enough data for {symbol}: got {len(df)} rows, need {lookback}")
            return None
        ...
        if curr > ema200:
            ...
            print(f"{symbol} below EMA200 (curr={curr:.2f}, ema200={ema200:.2f}), skipping.")
            return None
...
def volar_us_process():
    ...
    last_lookback = session.get('last_lookback_us', 55)
    ...
    if request.method == "POST":
        ...
        session['last_lookback_us'] = lookback
        ...
        if saved_path and os.path.exists(saved_path):
            ...
            raw_results = []
            for s in symbols:
                res = is_volar_adaptive(s, lookback)
            ...
            source_name = f"{session.get('last_filename_us')} ({lookback}d)"
            ...
            history.insert(0, {
                ...
                "source": session.get('last_filename_us'),
